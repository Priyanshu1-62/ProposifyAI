...
function RequestState(props: PropsWithChildren) {
  ...
  const getRequests = async () => {
    try {
        ...
        const result: apiResult = {ok: response.ok, status: response.status, data, success: "Requests fetched successfully !"};
        handleApiResponse(result);
        return result;
    } 
    catch (error) {
        handleAlert({color: "red", msg: "Something went wrong"});
        const result: apiResult = {ok: false, status: 500, data: {}, success: "Requests fetched successfully !"};
        return result;
    }
  }
